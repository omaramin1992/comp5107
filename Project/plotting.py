...
def plot_disc_func(m1, m2, cov1, cov2, x1_points, x2_points, p1, p2, d1, d2, method):
    ...
    plt.plot(x1_points[d1 - 1, :], x1_points[d2 - 1, :], 'b.', label="Class 1", alpha=0.5)
    plt.plot(x2_points[d1 - 1, :], x2_points[d2 - 1, :], 'r.', label="Class 2", alpha=0.5)
    plt.plot(equation_points, roots_2, 'g--', label="Dis.Fnc.")
    plt.plot(equation_points, roots_1, 'y--', label="Dis.Fnc.")
    plt.xlabel('x' + str(d1))
    plt.ylabel('x' + str(d2))

    # plt.axis([min_w - 0.5, max_w + 0.5, min_w - 0.5, max_w + 0.5])
    # plt.axis([min(min(x1_points[d1 - 1, :]), min(x2_points[d1 - 1, :]))-1,
